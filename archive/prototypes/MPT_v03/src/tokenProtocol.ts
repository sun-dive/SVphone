 ...
 *
 * A token is valid if:
 *   1. Token ID = SHA-256(genesisTxId || outputIndex LE)
...
/**
 * Compute Token ID = SHA-256(genesisTxId bytes || outputIndex as 4-byte LE).
 *
 * This is deterministic and requires no network access.
 */
export function computeTokenId(genesisTxId: string, outputIndex: number): string {
  ...
  const hash = sha256([...txIdBytes, ...indexBytes])
...
export function verifyMerkleProof(entry: MerkleProofEntry): boolean {
  let current = hexToBytes(entry.txId)
  ...
  return bytesToHex(current) === entry.merkleRoot
}
...
export async function verifyProofChainAsync(
  ...
  for (const h of heights) {
    try {
      ...
      return { valid: false, reason: `Failed to fetch header at height ${h}` }
    }
  }
 ...
 * @param genesisOutputIndex Output index in genesis TX (usually 0)
...
export function verifyToken(
  ...
  const expectedId = computeTokenId(genesisTxId, genesisOutputIndex)

...
class PhoneController {
    ...
    async init() {
        try {
            ...
            if (window.tokenBuilder && window.CallTokenManager) {
                this.callTokenManager = new CallTokenManager(window.tokenBuilder, (msg, type) => this.ui.log(msg, type))
    ...
    startBackgroundPolling() {
        ...
        if (!tokenBuilder || !myAddress) {
            ...
            this.ui.log(`   - tokenBuilder available: ${!!tokenBuilder}`, 'warning')
            this.ui.log(`   - myAddress value: ${myAddress ? myAddress.slice(0, 10) + '...' : 'not set'}`, 'warning')
        ...
        if (!this.signaling.myAddress) {
            ...
            this.ui.log(`[RECV]   ✓ Signaling initialized with address`, 'debug')
        } else {
            this.ui.log(`[RECV] ✓ Signaling already initialized with address: ${this.signaling.myAddress.slice(0,10)}...`, 'debug')
        }

        try {
            const controller = this  // Capture this context for logging

            // Define helper functions for polling
            const checkIncomingTokensFn = async (address) => {
                try {
                    controller.ui.log(`🔍 Polling for incoming tokens... (address=${address?.slice(0,10)}...)`, 'debug')
                    console.debug(`[RECV] Full polling address: ${address}`)

                    if (!address) {
                        controller.ui.log('[RECV] ❌ No address provided!', 'error')
                        return []
                    }

                    if (!tokenBuilder) {
                        controller.ui.log('[RECV] ❌ tokenBuilder not available!', 'error')
                        return []
                    }
                    ...
                    controller.ui.log(`✓ checkIncomingTokens() returned ${tokens?.length || 0} tokens`, 'debug')

                    if (!tokens || tokens.length === 0) {
                        controller.ui.log(`[RECV] ℹ️  No tokens found in this poll cycle`, 'debug')
                        return []
                    }

                    // Filter for call tokens where this address is the recipient
                    controller.ui.log(`[RECV] 🔎 Filtering ${tokens.length} tokens for incoming calls...`, 'debug')

                    const filtered = tokens.filter((t, idx) => {
                        // Check if token name is a call token
                        if (!t.tokenName || !t.tokenName.startsWith('CALL-')) {
                            return false
                        }

                        // Skip return-to-sender tokens
                        if (t.status === 'transferred' || t.status === 'pending_transfer') {
                            return false
                        }
                        ...
                        try {
                            ...
                            // Use signaling layer's parser to decode binary connection attributes
                            let attrs = {}
                            if (controller.signaling && controller.signaling.parseTokenAttributes) {
                                attrs = controller.signaling.parseTokenAttributes(t.tokenAttributes)
                            }

                            // Check if this address is the callee (incoming call) or caller (response)
                            if (!t.caller || !t.callee) {
                                return false
                            }

                            const isIncomingCall = t.callee === address
                            const isResponseToken = t.caller === address

                            return isIncomingCall || isResponseToken
                        } catch (e) {
                            controller.ui.log('[RECV]       ❌ Unexpected error parsing token attributes: ' + e.message, 'error')
                        }
                        return false
                    })

                    controller.ui.log(`[RECV] 📋 Filter result: ${filtered.length} of ${tokens.length} are incoming calls`, 'debug')
                    return filtered
                } catch (error) {
                    controller.ui.log('[RECV] Error checking tokens: ' + error.message, 'error')
                    return []
                }
            }
            ...
            const verifyTokenFn = async (token) => {
                try {
                    controller.ui.log(`[RECV] Verifying token ${token.tokenId?.slice(0,10)}...`, 'debug')
                    const result = await tokenBuilder.verifyToken(token.tokenId)
                    controller.ui.log(`[RECV] Verification result: valid=${result.valid}, reason=${result.reason || 'none'}`, 'debug')
                    return result
                } catch (error) {
                    controller.ui.log('[RECV] Error verifying token: ' + error.message, 'error')
                    return { valid: false, reason: error.message }
                }
            }

            // Start polling (5 second interval)
            this.ui.log(`[RECV] 📞 Starting background polling with address: ${this.signaling.myAddress?.slice(0,10)}...`, 'debug')
            this.signaling.startPolling(checkIncomingTokensFn, verifyTokenFn)
            this.ui.log(`[RECV] ✓ Polling initialized (interval: ${this.signaling.pollingInterval}ms)`, 'debug')

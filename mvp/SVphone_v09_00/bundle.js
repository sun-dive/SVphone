window.SVPHONE_BUILD="2026-03-06 12:27 UTC";document.addEventListener('DOMContentLoaded',()=>{const el=document.getElementById('svphone-build');if(el)el.textContent='build: 2026-03-06 12:27 UTC';});console.log('[SVphone] Build: 2026-03-06 12:27 UTC');
(() => {
...
class PhoneController {
        ...
        try {
            ...
            if (window.DtlsCertStore && window.ContactsStore && window.IceCredentials && window.SyntheticSdp) {
                ...
                this.dtlsCertStore.getOrCreate().then(cert => {
                    ...
                    if (addr) {
                        const identity = this.contactsStore.format(addr, fp)
    ...
    addContact() {
        ...
        if (!parsed) {
            this.ui.log('⚠️ Invalid identity string. Format: ADDRESS:sha-256:XX:XX:...', 'error')
            return
        }
        this.contactsStore.save(parsed.address, parsed.fingerprint)
        this.ui.log(`✓ Contact saved: ${parsed.address.slice(0, 16)}...`, 'success')
        input.value = ''
        this.refreshContactsList()
    }

 ...
 *
 * A token is valid if:
 *   1. Token ID = SHA-256(genesisTxId || outputIndex LE || opReturnChunks[4..6] raw bytes)
...
/**
 * Compute Token ID = SHA-256(genesisTxId || outputIndex LE || opReturnChunks[4..6] raw bytes).
 *
 * The immutableChunkBytes parameter is the concatenated raw bytes of
 * OP_RETURN chunks 4 (tokenName), 5 (tokenRules), and 6 (tokenAttributes).
 * This binds the token's identity to all immutable metadata.
 *
 ...
 * @param immutableChunkBytes  Concatenated raw bytes of OP_RETURN chunks 4-6
